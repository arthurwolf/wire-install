...
module.exports = class Remote {
    ...
    ssh(command) {
        //console.log(`ssh -o "StrictHostKeyChecking=no" -o "BatchMode=yes" wire@${this.options.ip} ${command} `);
        let result = sh.exec(`ssh -o "StrictHostKeyChecking=no" -o "BatchMode=yes" wire@${this.options.ip} ${command} `);
    ...
    run(command) {
        ...
        while (1) {
            ...
            if (/^(wire@(wire-(client|server)|kubenode01|arthur-demo)|bash-[\d\.-]*\#)/gi.test(lastline) && result.split("\n").length > 1) {
